...
Enforces the mandatory pre-flight execution order:
...
Usage (Multi-GPU DDP):
  torchrun --nproc_per_node=2 scripts/run_kaggle_stage1a.py --config configs/dhruva_v1_production.yaml
"""
...
def main():
    ...
    parser.add_argument("--preflight-only", action="store_true", help="Run only Steps A-D without starting Stage 1A")
    ...
    # (Preflight runs in-process across ranks)

    # ------------------------------------------------------------
    # STEP B & C: 20-Step FP16 DDP Smoke Test
    # ------------------------------------------------------------
    if rank == 0:
        print("\n>>> [STEP B & C] Running 20-step FP16 DDP Smoke Test...")
    ...
    # ------------------------------------------------------------
    # STEP D: Checkpoint Save / Load & Resume Verification
    # ------------------------------------------------------------
    ...
    if rank == 0:
        print(f"\n>>> [STEP D] Checkpoint Manager initialized at: {ckpt_dir}")
        print("    Backup Directory: " + str(cfg.get("output", {}).get("backup_dir", "./backup")))

    # ------------------------------------------------------------
    # STEP E: Stage 1A Pre-Training (Gated)
    # ------------------------------------------------------------
    if not args.execute_stage1a:
        if rank == 0:
            print("\n============================================================")
            print(" [STATUS] PRE-FLIGHT VERIFICATION COMPLETE — ALL GATES PASS.")
            print(" Stage 1A training was NOT started automatically.")
            print(" To launch full 100M token pre-training, re-run with:")
            print(f"   torchrun --nproc_per_node=2 scripts/run_kaggle_stage1a.py --config {args.config} --execute-stage1a")
            print("============================================================\n")
        if is_distributed:
            dist.destroy_process_group()
        return

    # Real Training Execution Path (Only when explicitly invoked with --execute-stage1a)
    if rank == 0:
        print(f"\n>>> [STEP E] LAUNCHING STAGE 1A 100M-TOKEN TRAINING...")

    # Load dataset
    ds_name = cfg.get("data", {}).get("dataset_name", "wikitext")
    ds_config = cfg.get("data", {}).get("dataset_config", "wikitext-2-raw-v1")
    tokenizer_path = out_dir / "tokenizer"

    tokenizer = BPETokenizer.load(str(tokenizer_path))
    train_dataset = load_and_tokenize_dataset(tokenizer, seq_len, dataset_name=ds_name, dataset_config=ds_config, split='train')
